 ...
 * \code
 * template<>
 * struct assertion_traits<std::string, const char *>  // specialization for the std::string type
 * {
...
/*! \brief Default traits base implementation. 
 * 
 * We put default implementation in the base template in case not everything needs to be
 * redefined by the specialized templates.
 */
template <class T1, class T2>
struct assertion_traits_base
{  
...
/*! \brief Default traits implementation. 
 * 
 * Specialize this template if == operator or << operator are not defined.
 */
template <class T1, class T2>
struct assertion_traits: public assertion_traits_base<T1, T2>
{
};

#if defined(STRINGAID_H)

/*! \brief Partial traits specialization for std::wstring type. 
 * 
 * For std::wstring type the << operator is not defined so default implementation can not translate
 * std::wstring into string. std::wstring to string translation is provided here.
 */
template <class T2>
struct assertion_traits<std::wstring, T2>: public assertion_traits_base<std::wstring, T2>
{
...
/*! \brief Partial traits specialization for L"" strings which are of wchar_t[N] type.
 * 
 * For const wchar_t[N] type the << operator is not defined so default implementation can not translate
 * const wchar_t[N] into string. const wchar_t[N] to string translation is provided here.
 */
template <size_t N, class T2>
struct assertion_traits<const wchar_t[N], T2>: public assertion_traits_base<const wchar_t[N], T2>
{
 ...
 */
template <>
struct assertion_traits<double, double>
{  
    static bool equal( double x, double y )
    {
        // return x == y;
        return (x - .0000001) <= y && (x + .0000001) >= y; 
    }
...
/*! \brief (Implementation) Asserts that two objects are of equal value.
                   ...
                   const std::string &message )
{
  ...
  {
    Asserter::failNotEqual( assertion_traits<T1, T1>::toString(expected),
                            assertion_traits<T2, T2>::toString(actual),
                            sourceLine,
                            message );
  }
}
