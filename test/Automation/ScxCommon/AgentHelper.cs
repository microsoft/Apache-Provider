...
namespace Scx.Test.Common
{
    ...
    public class AgentHelper
    {
        ...
        public void Install()
        {
            ...
            // Optionally uncompress agent file
            if (string.IsNullOrEmpty(this.decompressUtil) == false)
            {
                execInstall.FileName = string.Format(this.decompressUtil, "/tmp/" + agentName);
                this.logger("Install() decompress cmd line: " + execInstall.FileName);
                execInstall.RunCmd();
                this.logger("Install() decompress out: " + execInstall.StdOut);

                // Remove compression extension from agent name
                agentName = agentName.Remove(agentName.LastIndexOf(this.compressionExt));
                this.logger("Install() Uncompress, agentName=" + agentName);
            }
