...
namespace Scx.Test.Common
{
    ...
    public class RunPosixCmd : IRunHelper, IDisposable
    {
        ...
        protected delegate void PosixExecCmd(string command, out string result);
        ...
        public bool UseShellCommand
        {
            set
            {
                if (value == true)
                {
                    this.posixExecCmd = this.ssh.Execute;
                }
                else
                {
                    this.posixExecCmd = this.ssh.Execute;
        ...
        {
            uint retValue = this.ssh.GetSSH().OpenShell();
            if (retValue != 0)
            {
        ...
        public void RunShellCommand(string cmd)
        {
            ...
            this.ssh.GetSSH().SetTimeout((uint)this.timeOut);
            this.ssh.GetSSH().SetShellOutputTimeout((uint)this.timeOut);
        ...
        protected bool NewSshConnection()
        {
            ...
            try
            {
                this.ssh = new SshHelper(this.hostName, this.userName, this.password, this.port);
                this.posixExecCmd = this.ssh.Execute;
            }
            catch (Exception e)
            {
        ...
        protected void ExecuteSshCommand()
        {
            ...
            try
            {
                ...
                {
                    this.posixExecCmd(this.fileName, out this.stdout);
                }
                else
                {
                    this.posixExecCmd(this.fileName + " " + this.arguments, out this.stdout);
                }
            }
            catch (Exception e)
            {
        ...
        protected void ExecuteSshShellCommand()
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(this.followupPassword))
                {
                    ...
                    this.ssh.GetSSH().SetShellOutputTimeout(5000);

                    this.exitCode = this.ssh.GetSSH().ShellCommand2(this.fileName, out this.stdout);
                        ...
                        passwordIndex++)
                    {
                        ...
                        this.exitCode = this.ssh.GetSSH().ShellCommand2(followupPasswords[passwordIndex], out stdOutBuffer);
